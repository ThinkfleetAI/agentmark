/**
 * Tests for ActivepiecesMemoryBackend — request shape + response mapping.
...
describe('ActivepiecesMemoryBackend — construction', () => {
    it('requires baseUrl, sk-prefixed apiKey, and projectId', () => {
...
describe('ActivepiecesMemoryBackend — set', () => {
    it('looks for existing record + deletes it before creating new one', async () => {
        mockFetch((url, init) => {
            if (init?.method === 'GET') return jsonResponse([apItem({ id: 'mem_existing' })])
    ...
    it('JSON-stringifies non-string values; preserves raw value in metadata', async () => {
...
describe('ActivepiecesMemoryBackend — get + deleteByKey', () => {
    it('walks scopes in order; returns first match', async () => {
        let call = 0
        mockFetch(() => {
            call += 1
            if (call === 1) return jsonResponse([])
            return jsonResponse([apItem({ id: 'mem_platform' })])
        })
    ...
    it('disambiguates by scope_id when scope.id is supplied', async () => {
        mockFetch(() => jsonResponse([
            apItem({ id: 'mem_other', metadata: { agentmark_key: 'build_command', scope_id: '/repo/b' } }),
            apItem({ id: 'mem_mine', metadata: { agentmark_key: 'build_command', scope_id: '/repo/a' } }),
        ]))
        ...
        expect(record?.record_id).toBe('mem_mine')
    })
    ...
    it('deleteByKey returns false when no match', async () => {
        mockFetch(() => jsonResponse([]))
        const backend = new ActivepiecesMemoryBackend(baseConfig)
...
describe('ActivepiecesMemoryBackend — search', () => {
    it('routes text queries to /memory/search with scope filter', async () => {
        ...
        expect(captured?.body).toEqual({ query: 'how do I build', scope: 'project', limit: 25 })
    ...
    it('filters search results client-side by tags', async () => {
        mockFetch(() => jsonResponse([
            apItem({ id: 'mem_with', metadata: { agentmark_key: 'k1', tags: ['build', 'fast'] } }),
            apItem({ id: 'mem_without', metadata: { agentmark_key: 'k2' } }),
        ]))
        const backend = new ActivepiecesMemoryBackend(baseConfig)
        const results = await backend.search({ query: 'x', tags: ['build'] })
        expect(results).toHaveLength(1)
        expect(results[0].record_id).toBe('mem_with')
    })
})

describe('ActivepiecesMemoryBackend — list', () => {
    it('passes scope + source + limit; caps limit at 100', async () => {
        mockFetch(() => jsonResponse([apItem()]))
        const backend = new ActivepiecesMemoryBackend(baseConfig)
...
describe('ActivepiecesMemoryBackend — errors + describe', () => {
    it('throws ActivepiecesMemoryError on non-2xx with status + body', async () => {
        ...
        try {
            ...
            expect((err as ActivepiecesMemoryError).body).toEqual({ error: 'forbidden' })
        }
    })
    ...
    it('describe reports kind=activepieces + project + chatbot + source', async () => {
